...
import { RecoveryCase, RevenueType } from '@/lib/types';
...
export const RecoveryOpportunitiesTable: React.FC<RecoveryOpportunitiesTableProps> = ({
  ...
  subtitle = 'Prioritized by recovery probability, ERV, and optimal clearing window',
  limit,
  showViewAll = true,
}) => {
  ...
  const getSurfaceBadge = (type: RevenueType = 'payment') => {
    switch (type) {
      case 'checkout':
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded bg-info-50 text-info-700 border border-info-200 uppercase tracking-wider">
      ...
      case 'receivable':
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded bg-warning-50 text-warning-800 border border-warning-200 uppercase tracking-wider">
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded bg-brand-50 text-brand-700 border border-brand-200 uppercase tracking-wider">
        ...
        <table className="w-full text-left text-xs border-collapse">
          <thead>
            <tr className="bg-neutral-50 border-b border-neutral-200 text-neutral-500 uppercase tracking-wider text-[11px] font-semibold">
              <th className="py-3 px-5">Customer / Surface</th>
              <th className="py-3 px-4">Amount & ERV</th>
              <th className="py-3 px-4">Problem / Diagnostic</th>
              ...
              return (
                ...
                >
                  ...
                  <td className="py-3.5 px-5">
                    <Link href={`/recovery/${c.id}`} className="block focus:outline-none">
                      <div className="flex items-center gap-2">
                        ...
                        {getSurfaceBadge(c.revenueType)}
                      </div>
                      <span className="text-[11px] text-neutral-400 block mt-0.5">
                        {c.customer.name} · LTV ₹{(c.customer.lifetimeValue / 100000).toFixed(1)}L
                      </span>
                    </Link>
                  </td>

                  {/* Amount & Expected Recovery Value */}
                  <td className="py-3.5 px-4 whitespace-nowrap">
                    <span className="font-bold text-neutral-900 tabular-nums block">
                      {formatINRFull(c.amount)}
                    </span>
                    <span className="text-[10px] text-success-700 font-semibold tabular-nums block">
                      ERV: {formatINR(erv)}
                    </span>
                  </td>
                  ...
                  <td className="py-3.5 px-4 text-neutral-700 font-medium">
                    <span className="block font-semibold text-neutral-900">{c.rootCause}</span>
                    <span className="text-[11px] text-neutral-500 leading-tight">
                      {c.revenueType === 'receivable' && c.receivableDetails
                        ? `${c.receivableDetails.daysOverdue}d overdue (${c.receivableDetails.invoiceId})`
                        : c.revenueType === 'checkout' && c.checkoutDetails
                        ? `Dropped at ${c.checkoutDetails.abandonmentPoint}`
                        : c.payment.provider}
                    </span>
                  </td>
                    ...
                    <div className="flex items-center justify-end gap-2">
                      {c.status === 'at_risk' && (
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-2.5 py-1 bg-brand-50 hover:bg-brand-100 text-brand-700 rounded text-xs font-semibold border border-brand-200 transition-colors cursor-pointer"
                        >
            ...
            <div key={c.id} className="p-4 space-y-3">
              <div className="flex items-start justify-between">
                ...
                <div className="text-right">
                  ...
                  <span className="text-[10px] text-success-700 font-semibold block">
                    ERV: {formatINR(erv)}
                  </span>
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                {c.status === 'at_risk' && (
                  <button
                    onClick={() => executeStepOnCase(c.id)}
                    className="flex-1 py-2 bg-brand-500 hover:bg-brand-600 text-white rounded text-xs font-semibold transition-colors flex items-center justify-center gap-1.5 min-h-[44px] cursor-pointer"
                  >
